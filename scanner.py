...
class CognitiveScanner:
    ...
    def hierarchical_extract(self, file_path: Path, page_num: int = 0, doc_type: DocumentType = None) -> Dict[str, Any]:
        ...
        try:
            # V4.0 DELEGATION: Extraction Chain of Responsibility
            from integrations.extraction_chain import get_extraction_chain
            chain = get_extraction_chain(self.db)
            
            # Execute Chain (Sidecar -> Digital -> OCR -> Oracle)
            # This returns a clean dict with basic types
            chain_res = chain.process(file_path)
            
            # Merge into result structure
            result.update(chain_res)
            
            if result.get("doc_type"):
                 logger.info(f"Documento classificado como: {result['doc_type']} ({file_path.name})")
